...
def gaussian_blur(image, radius):
 blurred_image = cv2.GaussianBlur(image, (radius*2+1, 
radius*2+1), 0)
...
if uploaded_file is not None:
 # Read image
 image = cv2.imdecode(np.fromstring(uploaded_file.read(), 
np.uint8), 1)
...
["Grayscale", "Rotate", "Flip", "Gaussian Blur"])
...
caption='Grayscale Image', use_column_width=True)
 ...
 st.image(transformed_image, channels="BGR", caption='Rotated 
Image', use_column_width=True)
 
 elif transformation == "Flip":
 direction = st.radio("Select Direction", ["Horizontal", 
"Vertical"])
 # Apply flipping
 transformed_image = flip_image(image, direction)
 st.subheader("Flipped Image")
 st.image(transformed_image, channels="BGR", caption='Flipped 
Image', use_column_width=True)
 ...
 st.image(transformed_image, channels="BGR", caption='Blurred 
Image', use_column_width=True)
